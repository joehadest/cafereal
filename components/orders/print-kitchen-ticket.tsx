...
export function PrintKitchenTicket({ order, restaurantName, deliveryZoneName, newItemIds }: PrintKitchenTicketProps) {
  ...
  return (
    <div className="print-kitchen hidden print:block bg-white text-black font-mono overflow-visible font-bold" style={{ width: '100%', maxWidth: '100%', margin: '0', padding: '3mm 2mm', boxSizing: 'border-box', pageBreakInside: 'auto', height: 'auto', minHeight: 'auto', lineHeight: '1.4', fontSize: '13px', fontWeight: 'bold' }}>
      {/* Header - Destacado */}
      ...
      <div className="mb-2 pb-2 text-center" style={{ borderTop: '2px solid #000', borderBottom: '2px solid #000', padding: '4px 0', backgroundColor: '#e8e8e8' }}>
        <p className="font-bold uppercase" style={{ fontSize: '20px', fontWeight: 'bold' }}>
          ...
          {order.order_items.map((item, index) => {
            ...
            return (
              <div key={item.id} className={index < order.order_items.length - 1 ? "mb-2 pb-2" : ""} style={index < order.order_items.length - 1 ? { borderBottom: '1px dashed #ccc' } : {}}>
                ...
                <div className="mb-0.5" style={{ backgroundColor: '#f9f9f9', padding: '2px 3px', borderRadius: '1px' }}>
                  {item.category_name && (
                    <span className="uppercase font-bold" style={{ fontSize: '11px', color: '#666', fontWeight: 'bold' }}>[{item.category_name}] </span>
                  )}
                  <span className="font-bold uppercase" style={{ fontSize: '16px', fontWeight: 'bold' }}>
                ...
                {item.variety_name && (
                  <div className="ml-2 mb-0.5" style={{ backgroundColor: '#f0f0f0', padding: '1px 3px', borderRadius: '1px' }}>
                    <span className="font-bold uppercase" style={{ fontSize: '14px', fontWeight: 'bold' }}>TAMANHO: {item.variety_name.toUpperCase()}</span>
                  </div>
                )}
                {/* Extras */}
                {item.order_item_extras && item.order_item_extras.length > 0 && (
                  <div className="ml-2 mb-0.5" style={{ backgroundColor: '#f0f0f0', padding: '1px 3px', borderRadius: '1px' }}>
                    <span className="font-bold uppercase" style={{ fontSize: '13px', fontWeight: 'bold' }}>EXTRAS:</span>
                    {item.order_item_extras.map((extra) => (
                      <div key={extra.id} className="font-bold ml-1" style={{ fontSize: '13px', fontWeight: 'bold' }}>
      ...
      {/* Observações Gerais */}
      {order.notes && (
        <div className="mb-2 pb-2" style={{ borderTop: '2px solid #000', borderBottom: '2px solid #000', padding: '4px', backgroundColor: '#fff3e0' }}>
          <p className="font-bold uppercase mb-1" style={{ fontSize: '14px', fontWeight: 'bold' }}>⚠️ ATENÇÃO - OBSERVAÇÃO GERAL:</p>
          <p className="font-bold whitespace-pre-wrap" style={{ fontSize: '13px', fontWeight: 'bold', wordBreak: 'break-word', overflowWrap: 'break-word' }}>
            {order.notes}
          </p>
        </div>
      )}
      ...
      {(order.customer_name || order.payment_method || (isDelivery && order.customer_phone)) && (
        <div className="mb-2 pb-2" style={{ borderBottom: '1px dashed #000', backgroundColor: '#f5f5f5', padding: '3px 2px', borderRadius: '2px' }}>
          <p className="font-bold uppercase mb-1" style={{ fontSize: '14px', fontWeight: 'bold' }}>INFORMAÇÕES DO CLIENTE</p>
          {order.customer_name && (
          ...
          {isDelivery && deliveryZoneName && (
            <div className="mb-0.5">
              <span className="font-bold" style={{ fontSize: '13px', fontWeight: 'bold' }}>Zona: </span>
          ...
          {isDelivery && order.reference_point && (
            <div>
              <span className="font-bold" style={{ fontSize: '13px', fontWeight: 'bold' }}>Ponto de Ref.: </span>
              <span className="font-bold" style={{ fontSize: '13px', fontWeight: 'bold', wordBreak: 'break-word', overflowWrap: 'break-word' }}>
      ...
      <div className="text-center mt-2 pt-2" style={{ borderTop: '1px solid #000', backgroundColor: '#f5f5f5', padding: '3px 2px', borderRadius: '2px' }}>
        <p className="font-bold uppercase" style={{ fontSize: '13px', fontWeight: 'bold' }}>{restaurantName || "CAFEREAL"}</p>
      </div>
